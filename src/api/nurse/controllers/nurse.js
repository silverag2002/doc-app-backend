...
module.exports = createCoreController("api::nurse.nurse", ({ strapi }) => ({
  async create(ctx) {
    if (
      ctx.state.route.method == "POST" &&
      ctx.state.user.user_type < USER_CODE.NURSE &&
      ctx.state.user_type !== USER_CODE.DOCTOR
    ) {
      ...
      try {
        ...
        ctx.send({ message: "Nurse Created" });
      } catch (err) {
        console.log(err);
